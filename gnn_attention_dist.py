...
加入基于距离的注意力机制 (Distance-Based Attention)
保存文件名包含后缀 _gnn_attn
"""
import os
import numpy as np
...
displacements = disp_df["加载位移"].values[0:131]    # 131 个位移值
steps = list(range(5, 651, 5))
n_steps = len(steps)  # 应为 130，如果你取 131 请检查 disp 文件与 steps 一致性
...
distances, knn_idx = knn.kneighbors(coords_base) # distances 是距离矩阵

# *** MODIFIED ***
# 构造 edge_index (source, target) 和 edge_weights
edge_list = []
edge_weights_list = [] # *** NEW *** 用于存储边的权重

# *** NEW *** 计算距离的标准差，用于高斯核的带宽参数 sigma
# 为了使权重更稳定，我们可以将 sigma 设置为所有边距离的标准差
all_distances_for_sigma = distances.flatten()
sigma = np.std(all_distances_for_sigma)
print(f"Calculated sigma for Gaussian kernel: {sigma:.4f}")

for i in range(nodes_per_step):
    for j_idx, j in enumerate(knn_idx[i]):
        edge_list.append([i, j])
        # *** NEW *** 使用高斯核将距离转换为注意力权重
        # weight = exp(-distance^2 / (2 * sigma^2))
        dist = distances[i][j_idx]
        attention_weight = np.exp(-(dist ** 2) / (2 * sigma ** 2))
        edge_weights_list.append(attention_weight)

edge_index = torch.tensor(edge_list, dtype=torch.long).t().contiguous()  # shape [2, E]
edge_weights = torch.tensor(edge_weights_list, dtype=torch.float32).contiguous() # *** NEW *** shape [E]
...
# -------------------- 4. 带距离注意力的轻量 GCN 实现 --------------------
# *** MODIFIED ***
class GCNLayerWithAttention(nn.Module):
    def __init__(self, in_channels, out_channels, bias=True):
        super().__init__()
        self.linear = nn.Linear(in_channels, out_channels, bias=bias)

    # *** MODIFIED ***: 增加了 edge_weights 参数
    def forward(self, x, edge_index, edge_weights, deg_inv_sqrt):
        """
        x: [N, C_in]
        edge_index: [2, E] (source, target)
        edge_weights: [E,] 边的注意力权重
        ...
        # *** MODIFIED ***: 应用注意力权重
        weighted_x_j = x_j * edge_weights.unsqueeze(1) # [E, C]
        
        # symmetric normalization: m_ij = x_j * attn_weight * deg_inv_sqrt[row] * deg_inv_sqrt[col]
        norm = (deg_inv_sqrt[row].unsqueeze(1) * deg_inv_sqrt[col].unsqueeze(1))  # [E,1]
        msg = weighted_x_j * norm
...
# *** MODIFIED ***
class GNNModelWithAttention(nn.Module):
    def __init__(self, in_dim=3, hidden=64, out_dim=3, dropout=0.2):
        super().__init__()
        # 使用带注意力的GCN层
        self.gcn1 = GCNLayerWithAttention(in_dim, hidden)
        self.gcn2 = GCNLayerWithAttention(hidden, hidden)
        self.gcn3 = GCNLayerWithAttention(hidden, hidden)
        self.dropout = nn.Dropout(dropout)
        self.fc_out = nn.Linear(hidden, out_dim)

    # *** MODIFIED ***: 增加了 edge_weights 参数
    def forward(self, x, edge_index, edge_weights, deg_inv_sqrt):
        x = self.gcn1(x, edge_index, edge_weights, deg_inv_sqrt)
        x = self.dropout(x)
        x = self.gcn2(x, edge_index, edge_weights, deg_inv_sqrt)
        x = self.dropout(x)
        x = self.gcn3(x, edge_index, edge_weights, deg_inv_sqrt)
        out = self.fc_out(x)
        return out

# -------------------- 5. 训练设置 --------------------
# *** MODIFIED ***: 使用新的模型
model = GNNModelWithAttention(in_dim=3, hidden=64, out_dim=3, dropout=0.2).to(device)
edge_index = edge_index.to(device)
edge_weights = edge_weights.to(device) # *** NEW ***
...
for epoch in range(n_epochs):
    ...
    for batch in train_loader:
        ...
        optimizer.zero_grad()
        # *** MODIFIED ***: 传入 edge_weights
        pred = model(x_batch, edge_index, edge_weights, deg_inv_sqrt)  # [N, out_dim]
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            # *** MODIFIED ***: 传入 edge_weights
            pred = model(x_batch, edge_index, edge_weights, deg_inv_sqrt)
...
plt.legend()
plt.tight_layout()
plt.savefig('visualizations/loss_curve_gnn_attn.png', dpi=150) # *** MODIFIED ***
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig('visualizations/learning_rate_gnn_attn.png', dpi=150) # *** MODIFIED ***
plt.close()
...
with torch.no_grad():
    for step_idx in range(n_steps):
        ...
        # *** MODIFIED ***: 传入 edge_weights
        pred_scaled = model(x_step, edge_index, edge_weights, deg_inv_sqrt).cpu().numpy()               # [N, 3]
        ...
        # *** MODIFIED ***: 文件名加入 _attn
        result_df.to_csv(f"predictions/pred_step_{steps[step_idx]}_gnn_attn.csv", index=False, encoding='utf-8')
        ...
        if steps[step_idx] in list(range(0, 651, 100)):
            variables = ['U', 'V', 'Phi']
            units = ['位移(mm)', '位移(mm)', '']
            ...
            for i_var, (var, unit) in enumerate(zip(variables, units)):
                ...
                im2 = axes[1].imshow(grid_pred.T, extent=[x_min, x_max, y_min, y_max],
                                     cmap='RdBu_r', origin='lower', aspect='equal',
                                     vmin=combined_min, vmax=combined_max)
                axes[1].set_title(f'{var} 预测值 (Attn)\n范围: [{pred_original[:, i_var].min():.3e}, {pred_original[:, i_var].max():.3e}] {unit}') # *** MODIFIED ***
                ...
                plt.tight_layout()
                # *** MODIFIED ***: 文件名加入 _attn
                plt.savefig(f'visualizations/step_{steps[step_idx]}_{var}_gnn_attn_analysis.png', dpi=150, bbox_inches='tight')
                plt.close()
...
plt.tight_layout()
plt.savefig('visualizations/error_vs_displacement_gnn_attn.png', dpi=150) # *** MODIFIED ***
plt.close()
...
plt.tight_layout()
plt.savefig('visualizations/error_distribution_gnn_attn.png', dpi=150) # *** MODIFIED ***
plt.close()
...
plt.title('带注意力机制的 GNN 整体误差对比') # *** MODIFIED ***
plt.tight_layout()
plt.savefig('visualizations/overall_error_comparison_gnn_attn.png', dpi=150) # *** MODIFIED ***
plt.close()

# -------------------- 9. 输出结果 --------------------
print("\n预测结果已保存到 predictions/ 文件夹中（每步 CSV，文件名含 _gnn_attn）")
print("可视化图表已保存到 visualizations/ 文件夹中（文件名含 _gnn_attn）")
print("\n整体预测误差统计 (GNN with Attention):") # *** MODIFIED ***
