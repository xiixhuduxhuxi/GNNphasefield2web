...
displacements = displacement_df["加载位移"].values[0:131]
...
# 4. MLP模型定义
class MLPModelWithAttention(torch.nn.Module):
    def __init__(self, input_dim=3, output_dim=3, dropout=0.2):
        super().__init__()
        self.fc1 = torch.nn.Linear(input_dim, 64)
        self.attn1 = FeatureAttention(64, reduction=8)
        
        self.fc2 = torch.nn.Linear(64, 128)
        self.attn2 = FeatureAttention(128, reduction=8)
        
        self.fc3 = torch.nn.Linear(128, 64)
        self.attn3 = FeatureAttention(64, reduction=8)
        
        self.fc4 = torch.nn.Linear(64, 32)
        self.attn4 = FeatureAttention(32, reduction=4)
...
# 5. 模型训练
model = MLPModelWithAttention().to(device)
...
plt.tight_layout()
plt.savefig('visualizations/loss_curve_mlp_attention.png', dpi=150)
plt.close()
...
plt.legend(fontsize=12)
plt.tight_layout()
plt.savefig('visualizations/learning_rate_mlp_attention.png', dpi=150)
plt.close()
# 7. 预测与可视化
print("\n开始生成预测结果...")
model.eval()
...
with torch.no_grad():
    for step_idx in all_step_indices:
        ...
        if step in list(range(0, 651, 100)):
            variables = ['U', 'V', 'Phi']
            units = ['位移(mm)', '位移(mm)', '']
            ...
            for i, (var, unit) in enumerate(zip(variables, units)):
                ...
                # 移除顶部标题
                # fig.suptitle(f'步骤 {step}, 位移: {disp:.3f} | {var} 分量预测结果 (带注意力机制)', 
                ...
                plt.tight_layout()
                plt.savefig(f'visualizations/step_{step}_{var}_mlp_attention_analysis.png', dpi=150, bbox_inches='tight')
                plt.close()
...
for i, var in enumerate(variables):
    ...
    # 移除顶部标题
    # plt.title('误差随位移变化趋势 (带注意力机制)', fontsize=14)
    plt.grid(True, linestyle='--', alpha=0.7)
...
plt.tight_layout()
plt.savefig('visualizations/error_vs_displacement_mlp_attention.png', dpi=150)
plt.close()
...
for i, (var, color) in enumerate(zip(variables, colors)):
    ...
    # 移除顶部标题
    # plt.title(f'{var} 误差分布 (log10)', fontsize=12)
    plt.xlabel('log10(误差值)', fontsize=10)
    plt.ylabel('频数', fontsize=10)
    plt.grid(True, linestyle='--', alpha=0.7)

plt.tight_layout()
plt.savefig('visualizations/error_distribution_mlp_attention.png', dpi=150)
plt.close()
...
# 移除顶部标题
# plt.title('各变量预测误差对比 (带注意力机制)', fontsize=14)
...
plt.tight_layout()
plt.savefig('visualizations/overall_error_comparison_mlp_attention.png', dpi=150)
plt.close()
